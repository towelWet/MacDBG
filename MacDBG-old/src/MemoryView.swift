...
struct MemoryView: View {
    ...
    // Convert memory dictionary to MemoryLine array
    private var memoryLines: [MemoryLine] {
        debugger.memory.map { (address, bytes) in
            let addressStr = String(format: "0x%016llX", address)
            let bytesStr = bytes.map { String(format: "%02X", $0) }.joined(separator: " ")
            let asciiStr = bytes.map { byte in
                if byte >= 32 && byte <= 126 {
            ...
            return MemoryLine(address: addressStr, bytes: bytesStr, ascii: asciiStr)
        }.sorted { line1, line2 in
            let addr1 = UInt64(line1.address.dropFirst(2), radix: 16) ?? 0
            let addr2 = UInt64(line2.address.dropFirst(2), radix: 16) ?? 0
            return addr1 < addr2
        }
    }
            ...
            HStack {
                ...
                if debugger.isAttached {
                    HStack {
                        ...
                        Button("Read") {
                            Task { 
                                if let address = UInt64(addressInput.replacingOccurrences(of: "0x", with: ""), radix: 16) {
                                    await debugger.readMemory(address: address, bytes: bytesToRead)
                                }
                            }
                        }
    ...
    private func handleMemorySelection(address: String, isSelected: Bool) {
        if isSelected {
            ...
            if NSEvent.modifierFlags.contains(.shift), let start = selectionStart {
                ...
                let sortedAddresses = memoryLines.map { $0.address }
    ...
    private func copySelectedMemory() {
        ...
        let selectedMemory = memoryLines.filter { sortedSelected.contains($0.address) }
            .sorted { line1, line2 in
