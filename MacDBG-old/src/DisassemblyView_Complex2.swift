    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingGotoDialog) {
            VStack {
                ...
                TextField("Address", value: $gotoAddress, format: .number)
                ...
                HStack {
                    ...
                    Button("Goto") {
                        Task {
                            await debugger.getDisassemblyAt(address: gotoAddress)
                            showingGotoDialog = false
                        }
                    }
                }
    ...
    private func handleDoubleClick(address: UInt64) {
        ...
        Task {
            if debugger.breakpoints.contains(address) {
                await debugger.removeBreakpoint(at: address)
            } else {
                await debugger.addBreakpoint(at: address)
            }
        }
    }
}
