...
struct GPRView: View {
    ...
    private var gprRegisters: [(String, String)] {
        ...
        return gprNames.compactMap { name in
            if let value = debugger.registers[name.lowercased()] ?? debugger.registers[name] {
                return (name, String(format: "0x%016llX", value))
            }
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 2) {
                    ForEach(Array(debugger.memory.keys.sorted().prefix(20)), id: \.self) { address in
                        if let bytes = debugger.memory[address] {
                            HStack(spacing: 8) {
                                Text(String(format: "0x%016llX", address))
                                    .font(.system(.caption, design: .monospaced))
                                    .foregroundColor(.secondary)
